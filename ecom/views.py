...
def add_to_cart_view(request, pk):
    customer_id = request.user.id  # Get the current logged-in user's ID
    cookie_key = f'product_ids_{customer_id}'  # Unique key for each customer

    products = models.Product.objects.all()

    # For cart counter
    if cookie_key in request.COOKIES:
        product_ids = request.COOKIES[cookie_key]
        counter = product_ids.split('|')
        product_count_in_cart = len(set(counter))
    else:
    ...
    if cookie_key in request.COOKIES:
        ...
        response.set_cookie(cookie_key, product_ids)
    else:
        response.set_cookie(cookie_key, pk)
...
def remove_from_cart_view(request, pk):
    ...
    if cookie_key in request.COOKIES:
        ...
        if value == "":
            response.delete_cookie(cookie_key)  # Clear the cookie if no products are left in the cart
        else:
...
def customer_home_view(request):
    customer_id = request.user.id  # Get the current logged-in user's ID
    cookie_key = f'product_ids_{customer_id}' 
    products=models.Product.objects.all()
    if cookie_key in request.COOKIES:
        product_ids = request.COOKIES[cookie_key]
        counter=product_ids.split('|')
